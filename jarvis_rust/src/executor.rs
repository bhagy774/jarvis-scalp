...
impl OrderExecutor {
    ...
    pub fn place_order(
        ...
        if order_type_lower != "limit" && order_type_lower != "market" {
            return Err(PyValueError::new_err("order_type must be 'limit' or 'market'"));
        }
    ...
    pub fn cancel_order(&self, py: Python<'_>, order_id: String, symbol: String) -> PyResult<PyObject> {
        let path = "/v2/orders";
        let timestamp = now_timestamp();
        let body_json = json!({
        ...
        let response: PyResult<Value> = self.runtime.block_on(async move {
            ...
            resp.json::<Value>()
                .await
                .map_err(|e| PyRuntimeError::new_err(format!("Failed to parse cancel response: {e}")))
        });
