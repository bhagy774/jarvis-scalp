        ...
        if self._binance:
            try:
                ...
                if candles and len(candles) > 0:
                    semantics = str(candles[0].get("market_semantics", "spot")).lower()
                    if semantics == "spot" or self._perpetual_fallback_allowed():
                        return candles
                    logger.warning("[HYBRID] Rejecting non-spot fallback candles for %s; explicit compatibility is required", symbol)
            except Exception as e:
